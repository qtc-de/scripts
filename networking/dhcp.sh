...
function enable_forwarding()
{
    ...
    sudo nft "add rule inet firewall forward iifname ${IF_FROM} oifname ${IF_TO} accept"
    sudo nft "add rule inet firewall forward ct state established,related accept"

    sudo nft 'add chain inet firewall nat { type nat hook postrouting priority 100 ; }'
    sudo nft "add rule inet firewall nat iifname ${IF_FROM} oifname ${IF_TO} masquerade"
